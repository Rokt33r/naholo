...
The eleven core skills (`/infil`, `/warno`, `/raid`, `/opord`, `/splash`, `/sitrep`, `/exfil`, `/chop`, `/chopchop`, `/nochop`, `/recon`) are bundled into the `naholo` CLI. The CLI ships them into the user's `.claude/skills/` directory via `naholo install-skills`.
