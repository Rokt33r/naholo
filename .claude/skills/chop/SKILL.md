...
The first half of the OP-splitting workflow. `/chop` reads the parent's current WARNING ORDER + OPERATION ORDER, takes a freeform description of what to carve, and writes a `CHOP.md` proposal note showing **side-by-side** how Constraints and tasks split between the current OP and a proposed new OP. The user reviews (and optionally edits) `CHOP.md`, then runs `/chopchop` to apply it.

CHOP — short for **Change of Operational Control** — is the military verb for transferring a unit from one commander to another. Here the "unit" is a slice of WARNING ORDER + OPERATION ORDER: a set of Constraints and the unfinished tasks that depend on them. `/chop` is the planning brief; `/chopchop` is the actual transfer.
...
- `{operationDir}/notes/OPERATION.md` — the live OP document; re-read every invocation so manual mid-session edits land; the parent's `## SITUATION` is source material for the two proposed SITUATIONs drafted in step 3, and the rest is needed to identify carveable Constraints / tasks and each task's shipped state
...
If `## WARNING ORDER` is absent in `OPERATION.md`, stop and tell the user there's nothing to chop yet — run `/warno` first.
...
- **Carved Constraints** — the exact Constraint bullets (by bold label) that match the args. If args are ambiguous, pick the smallest literal-reading set and surface the ambiguity in `## Intent`.
- **Surviving Constraints** — every Constraint that's not carved.
- **Carved tasks** — every `### TASK N — Title` whose Intent / Scheme of Maneuver only makes sense under the carved Constraints. If any candidate carved task is **shipped** (its parent section has `#### After-Action Report`), pause and ask the user via `AskUserQuestion` whether it's intentional to move shipped work to the new OP. Show the task number + title + a one-line AAR summary so they can decide per task. Default option: keep shipped tasks on the parent. Only include a shipped task under `# NEW OP` if the user explicitly confirms.
...
- **Proposed new-OP title** — short noun phrase, follows the parent's title conventions. Use the user's suggestion if their args included one; otherwise propose one based on the carved Constraints.
- **Parent SITUATION (post-split)** — narrow the parent's existing `### Pain` body to the surviving scope, in the parent's own words where they still read true. Drop `### Notes` bullets that only applied to carved Constraints; keep ones that still apply. Omit the `### Notes` heading entirely when nothing survives (same convention as `OPERATION.md`).
...
- **Concept of Operations for the new OP** — **two or three sentences max**, scoped to the carved approach. Concept-level only: name the chosen path and connect it back to the carved Pain. Do **not** enumerate files, edit steps, or build commands here — those belong in Constraints or land later in OPERATION ORDER on the new OP. May be a verbatim slice of the parent's CoO if it already reads narrowly.

**Revision (`CHOP.md` already existed)** — start from the existing proposal and edit only the fields the args explicitly target. Do **not** re-derive every field from the parent — that would clobber the user's review pass. The default for every field is "leave it alone." Apply the args as targeted edits — move a Constraint between sides, retitle the new OP, rewrite a Concept of Operations, swap two tasks, edit a SITUATION body — using the fresh-draft field definitions above as the spec for what each field means.
...
{One to three sentences at most. Concept-level only — names the cleave line and why. No file lists, no edit steps, no TRP, no enumerated Constraint references. Surface any ambiguity from step 3 here in plain language.}
...
{The surviving Constraints, verbatim — preserve bold labels and reasoning halves. **Drop** any `- ?` open-alt and `- Rejected:` sub-bullets; they don't belong in a CHOP planning brief.}
...
{The carved Constraints, verbatim — preserve bold labels and reasoning halves. **Drop** any `- ?` open-alt and `- Rejected:` sub-bullets; they don't belong in a CHOP planning brief.}
...
- **No `#### Intent`, no `#### Scheme of Maneuver`, no `#### After-Action Report`** anywhere in `CHOP.md`. The OPERATION ORDER blocks under each OP are **checkbox task items** (`- [ ] TASK n — title` or `- [x] TASK n — title`), not full task sections. Full per-task detail lives in `OPERATION.md`; `CHOP.md` is a planning brief.
...
- **Constraints transfer verbatim except for `- ?` and `- Rejected:` sub-bullets, which are always dropped.** Bold labels and reasoning halves stay word-for-word; open alts and rejected-option lines do not appear in `CHOP.md`.
...
```
naholo agent add-timeline -T chop 'Drafted CHOP.md: carved {N} constraint(s), {M} task(s); proposed new OP "{proposed title}".'
```

or, on a revision run:

```
naholo agent add-timeline -T chop 'Revised CHOP.md: {one-sentence summary of what changed — e.g. "moved constraint X to the new OP", "retitled new OP", "rewrote new OP CoO"}.'
```
...
- Surviving: {N} constraint(s), {M} task(s) ({S} shipped, immutable)
...
- **No per-task detail in `CHOP.md`** — OPERATION ORDER blocks are one-line task summaries only. No Intent, Scheme of Maneuver, or AAR.
