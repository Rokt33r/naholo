...
function main(): void {
  const level = parseBumpLevel()
  const repoRoot = process.cwd()
  const buildScriptPath = path.join(repoRoot, 'scripts/build-claude-plugin.ts')

  assertCleanTree(repoRoot)

  const version = bumpPluginVersion(buildScriptPath, level)
  const tag = `naholo-claude-plugin@${version}`
  commitAndTag(repoRoot, buildScriptPath, tag)

  console.log(`\nBumped naholo-claude-plugin to ${version} and tagged ${tag}.`)
}

function parseBumpLevel(): string {
...
function assertCleanTree(repoRoot: string): void {
...
function bumpPluginVersion(buildScriptPath: string, level: string): string {
  const source = fs.readFileSync(buildScriptPath, 'utf8')
  const match = source.match(/const PLUGIN_VERSION = '([^']+)'/)
  if (match == null) {
    console.error(
      `Could not find a PLUGIN_VERSION constant in ${path.relative(process.cwd(), buildScriptPath)}.`,
    )
    process.exit(1)
  }
