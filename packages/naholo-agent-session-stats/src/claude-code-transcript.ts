// Format-agnostic raw transcript parsing. Per-format aggregation lives in
// sibling files like `claude-code-v1.ts`.
...
export type ClaudeCodeTranscriptEntry = {
  ...
  toolUses: string[]
  usage: ClaudeCodeTokenUsage | null
  summary: string | null
  raw: unknown
}
...
function extractUsage(
  ...
  if (cacheCreationSplit != null && typeof cacheCreationSplit === 'object') {
    ...
    // Older transcripts might only carry the flat `cache_creation_input_tokens` total
    cache1h = numberOrZero(u.cache_creation_input_tokens)
  }
