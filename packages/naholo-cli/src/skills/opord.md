...
- **Title is a commit subject.** A task ships as one commit, so its title is that commit's subject: write it in imperative command form ("Add storage adapter", not "Adding…" or "Storage adapter"), capped at 50 characters. If the project documents its own commit-subject length, that rule wins.
