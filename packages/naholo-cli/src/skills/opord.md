...
The skill name is the unambiguous "where are we" signal: re-running `/opord` is for plan adjustment (split / merge / retitle / rewrite unfinished tasks, full restart). Mid-cycle insertion of a single new task — slotting one in next to the last completed task — is `/frago`'s job, not `/opord`'s. Direction changes — Concept of Operations rewrites, Warning Order revisions — belong to `/warno`, not `/opord`.
...
For inserting a single new task next to the last completed task, use `/frago` — it's the dedicated insertion skill. MISSION-shaped instructions (Concept of Operations rewrite, Warning Order changes) belong to `/warno`, not `/opord`.
...
- **Args provided, classify intent**:
  ...
  - **Multi-task revision** — args describe removing or rewriting multiple unfinished tasks at once. Apply the described edits; renumber subsequent unfinished tasks as needed. Never delete or rewrite a task whose `#### After-Action Report` heading is present. Append `- **{YYYY-MM-DD HH:MM} — opord (revised)**: {summary}` to TIMELINE.md. (For a single insertion next to the last completed task, redirect the user to `/frago`.)
...
- Plan adjustment → re-run `/opord "freeform instructions"` or edit EXECUTION directly
- Insert a new task next to the last completed one → run `/frago "freeform instructions"`
...
- **Single-task insertion is `/frago`'s job**: if the user asks for "insert a new task" or similar, point them at `/frago` rather than appending here.
