...
const updateCommand = new Command('update')
  ...
  .action(
    async (skillSetSlug: string, options: { name?: string; slug?: string }) => {
      if (options.name == null && options.slug == null) {
        console.error('Provide at least one of --name or --slug')
        process.exit(1)
      }
      ...
      await client.updateSkillSet(projectConfig.projectId, skillSetSlug, input)
...
const deleteCommand = new Command('delete')
  ...
  .action(async (skillSetSlug: string) => {
