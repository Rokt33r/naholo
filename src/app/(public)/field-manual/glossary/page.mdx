...
Naholo's vocabulary is borrowed from OPORD (operations order) doctrine — the five-paragraph order the U.S. military uses to brief a mission. Each entry below names the military origin briefly, then how the concept maps into the naholo cycle.
...
**Naholo.** `/warno` writes the MISSION section of `OPERATION.md` — the direction the cycle commits to before any tasks get cut.
...
**Naholo.** `/raid` collapses `/warno` + `/opord` for small ops where architecture review would be overkill — real CONOPS and TRP, but Warning Orders marked `_N/A_`.
...
**Naholo.** `/opord` cuts MISSION into single-commit-sized tasks and writes the EXECUTION section of `OPERATION.md`. Each task carries an Intent, an optional Scheme of Maneuver, and a Course of Action.
...
**Naholo.** The `/chop` → `/chopchop` / `/nochop` family splits one OP into two when scope has ballooned — carving Warning Orders and tasks off to a new OP under a new commander.
...
**Naholo.** `## SITUATION` in `OPERATION.md`. Three subsections — Pain, Suggested solution (optional), Notes (optional). Seeded by `/infil` from the OP's logs and notes.

<h3 id='mission'>MISSION</h3>

**Military.** The second paragraph of the OPORD. What the unit will do. The commitment.

**Naholo.** `## MISSION` in `OPERATION.md`. Three subsections — Concept of Operations, Warning Orders, Target Reference Points. Written by `/warno`.
...
**Naholo.** The two-or-three-sentence overview at the top of MISSION naming the chosen approach and tying it back to SITUATION's pain. Concept-level only — no file lists.
...
**Naholo.** A curated, scannable map of files / folders / globs a fresh `/opord` session needs to read to cut EXECUTION. Lives at the tail of MISSION as `### Target Reference Points`.

<h3 id='execution'>EXECUTION</h3>

**Military.** The third paragraph of the OPORD. How the mission gets done — task organization, scheme of maneuver, sub-unit tasks, coordinating instructions.

**Naholo.** `## EXECUTION` in `OPERATION.md`. One `### TASK N — Title` per task, in shipping order. Written by `/opord`.

<h3 id='som'>SOM — Scheme of Maneuver</h3>
...
**Naholo.** Optional per-task `#### Scheme of Maneuver` in EXECUTION — an ASCII diagram of control flow, a UI wireframe, a schema column listing, or a before-after signature diff. Required when the task changes control flow, UI layout, public signatures, DB schema, DTOs, or API shapes; skipped on pure data/logic edits.

<h3 id='coa'>COA — Course of Action</h3>

**Military.** A planned series of actions that accomplish a task or mission. The commander picks the best COA before issuing the order.

**Naholo.** The per-task action list inside EXECUTION — `Add` / `Edit` / `Move` / `Delete` / `Run` / `Manual` steps that `/splash` walks when it ships the task.
