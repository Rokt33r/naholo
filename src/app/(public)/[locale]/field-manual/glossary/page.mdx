...
**Naholo.** `/recon` is the talk-it-out side branch: it answers questions and records a change request as a pinned RECON note (`Intent`, optional `Scheme`, `FRAGMENTARY ORDER`) that drafts the next `/warno` / `/opord` / revision `/splash` edit. The note is the review surface; the user fires the routed skill to apply it.
...
<h3 id='frago'>FRAGO — Fragmentary Order</h3>

**Military.** A brief change to a standing operations order: an amendment issued mid-mission without rewriting the whole OPORD.

**Naholo.** `/frago` applies a combined WARNO + OPORD revision in one pass, chaining `/warno` then `/opord` with the same prompt. The natural applier of a RECON note's FRAGMENTARY ORDER.
