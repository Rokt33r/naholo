...
export function TaskProvider({
  ...
  const { data: tasks = [], isLoading } = useTasks(projectSlug, operationNumber)
  const createTaskMutation = useCreateTask(projectSlug, operationNumber)
  const updateTaskMutation = useUpdateTask(projectSlug, operationNumber)
  const setTaskDoneMutation = useSetTaskDone(projectSlug, operationNumber)
  const deleteTaskMutation = useDeleteTask(projectSlug, operationNumber)
  const updateTaskNoteMutation = useUpdateTaskNote(projectSlug, operationNumber)
  const moveTaskMutation = useMoveTask(projectSlug, operationNumber)
  ...
  const indentTask = useCallback(
    async (taskId: string) => {
      ...
      const newSiblings = getChildTasks(previousSibling.id)
    ...
    [getPreviousSibling, getChildTasks, moveTaskMutation],
  )
